...
```
Vamos a realizar la predicción sobrel el incremento de contaminación, por lo tanto lo que hacemos es generar una nueva variable que indicael incremento del contaminante sobre el día anterior
...
normalizamos los datos eliminando la primera fila que no tiene incremento
```{r}
dataNuevo <- data[!is.na(data$incremento),]
dataNorm <- scale(dataNuevo[,2:15])
```
...
```{r}
dataModelo <- dataNorm[, c(1,2,3,4,6,7,9,14)]
...
```
Vamos a preparar los datos para el entrenamiento y el test
```{r}
n <- nrow(dataArima)
...
```{r}
modeloArima<-auto.arima(train)
summary(modeloArima)
```
```{r}
pronostico<- forecast(modeloArima,h=10)
plot(pronostico, main="Pronóstico con auto.arima", ylab="incremento NO2")
```
...
```{r}
redNeuronal <- neuralnet(formula = 'cont8 ~ est3491 + est6809 + est4451 + est4138 + presMax + presMin + velmedia' ,data=train,  hidden=c(6,5,3,2), linear.output=TRUE, threshold=0.05)
```
